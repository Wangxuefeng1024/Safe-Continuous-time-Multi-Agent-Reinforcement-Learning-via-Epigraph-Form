...
class Scenario(BaseScenario):
    ...
    def continuous_penalty(self, world, agent, scale_ao=20.0, scale_aa=20.0, eps=1e-9):
        """
        对“当前 agent”计算连续碰撞代价：
        - agent–obstacle：hinge = max(0, (r_a + r_o) - d)
        - agent–agent：   hinge = max(0, (r_a + r_b) - d)
        返回的是“正的代价”，无碰撞时为 0。
        """
        ...
        for obs in world.landmarks[self.num_agents:]:
            ...
            hinge = (ra + ro) - d
            if hinge > 0.0:
                total += scale_ao * hinge  # 只在碰撞/重叠时计入

        # --- agent–agent ---
        for other in world.agents:
            if other is agent:
                continue
            pb = other.state.p_pos
            rb = other.size
