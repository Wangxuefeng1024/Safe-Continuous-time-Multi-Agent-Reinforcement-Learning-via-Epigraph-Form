...
from continuous_env.Safe_Mujoco.safety_multi_agent_mujoco.safety_ma_mujoco.safety_multiagent_mujoco import mujoco_env
...
class HalfCheetahEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step_con(self, action, dt):
        ...
        self.do_simulation(action_p_wall, int(dt//0.01*self.frame_skip))
        ...
        # print("xdist", xdist)
        obj_cost = int(np.abs(xdist) < 1)
        distance = xposafter - (wallpos -1) 
        if distance >= 0:
            continuous_cost = 10 * distance
        else:
            continuous_cost = 0.2 * distance
        ...
        return ob, reward, done, dict(cost=cost, reward_run=reward_run, reward_ctrl=reward_ctrl, cost_obj = -obj_cost, continuous_cost=continuous_cost)
