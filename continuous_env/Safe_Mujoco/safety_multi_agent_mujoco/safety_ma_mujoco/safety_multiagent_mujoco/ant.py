...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step_con(self, a, dt):
        xposbefore = self.get_body_com("torso")[0]
        # print(int(dt/0.01*self.frame_skip))
        self.do_simulation(a, int(dt/0.01*self.frame_skip))
        mjp.functions.mj_rnePostConstraint(self.sim.model,
                                           self.sim.data)  #### calc contacts, this is a mujoco py version mismatch issue with mujoco200
        xposafter = self.get_body_com("torso")[0]
        forward_reward = (xposafter - xposbefore) / dt
        reward_cost = 1.0 / (1.0 + np.exp(-1 * (xposbefore - xposafter)))  
        ctrl_cost = .5 * np.square(a).sum()
        contact_cost = 0.5 * 1e-3 * np.sum(
            np.square(np.clip(self.sim.data.cfrc_ext, -1, 1)))
        survive_reward = 1.0
        ...
        ### ADDED continuous cost
        distance = 1.8 - abs(y_walldist)
        dis1, dis2 = distance
        if dis1 >= 0 or dis2 >= 0:
            continuous_cost = -20 * dis1* dis2
        else:
            continuous_cost = - dis1* dis2
        ...
        return ob, reward, done, dict(
            ...
            cost_done=done_cost,
            cost=cost,
            reward_cost= reward_cost, #ADDED
            continuous_cost=continuous_cost, #ADDED
